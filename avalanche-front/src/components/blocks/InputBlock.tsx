...
export const InputBlock: React.FC<InputBlockProps> = ({ 
  ...
  // Don't auto-populate subnet owner field to preserve P-chain addresses
  // The subnet owner should be a P-chain address, not a C-chain wallet address
  useEffect(() => {
    // Disabled auto-population for Subnet Owner to preserve P-chain addresses
    // if (
    ...
    //   onChange
    // ) {
    //   // Only auto-populate if there's no existing value and no P-chain address in defaults
    //   // Don't replace P-chain addresses with C-chain addresses
    //   const isPChainAddress = value && value.startsWith('P-');
    //   if (!isPChainAddress) {
    //     onChange(address);
    //   }
    // }
    ...
    <div className="mb-4">
      ...
      <input
        ...
        className={`w-full px-3 py-2 bg-gray-800 text-lg border rounded-lg text-white placeholder-gray-400 focus:outline-none transition-colors ${
          isSubnetOwnerField && isPChainAddress
            ? 'border-green-500 focus:border-green-400'
            : 'border-gray-600 focus:border-[#ff394a]'
        }`}
      />
      
      {/* P-chain Address Helper for Subnet Owner */}
      {isSubnetOwnerField && !isPChainAddress && (
        <div className="mt-2 text-xs text-blue-400 flex items-center gap-1">
          <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
          </svg>
          Subnet owner should be a P-chain address (starts with "P-")
        </div>
      )}
      
      {/* P-chain Address Confirmation for Subnet Owner */}
      {isSubnetOwnerField && isPChainAddress && (
        <div className="mt-2 text-xs text-green-400 flex items-center gap-1">
          <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
          </svg>
          Valid P-chain address format
        </div>
      )}
    </div>
  );
